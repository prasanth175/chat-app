...
exports.sendFcm = functions.region('europe-west3').https.onCall (async (data, context) => {
    ...
    const tokens = userTokenResult.reduce((accTokens, userTokens) => [...accTokens, ...userTokens], []);
    ...
    const removePromises = failedTokens.map(taken => database.ref(`/fcm_tokens/${token}`).remove())
